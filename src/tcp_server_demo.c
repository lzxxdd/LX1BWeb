...
unsigned char tcp_server_recvbuf[TCP_SERVER_RX_BUFSIZE];	//TCP客户端接收数据缓冲区
unsigned char *tcp_server_sendbuf="Explorer STM32F407 NETCONN TCP Server send data\r\n";
unsigned char tcp_server_flag;								//TCP服务器数据发送标志位
...
//任务堆栈
unsigned int TCPSERVER_TASK_STK[TCPSERVER_STK_SIZE];
...
static void tcp_server_thread(void *arg)
{
	...
	while (1) 
	{
		...
		{ 
			...
			remot_addr[3] = (unsigned char)(ipaddr.addr >> 24);
			remot_addr[2] = (unsigned char)(ipaddr.addr>> 16);
			remot_addr[1] = (unsigned char)(ipaddr.addr >> 8);
			remot_addr[0] = (unsigned char)(ipaddr.addr);
...
//返回值:0 TCP服务器创建成功
//		其他 TCP服务器创建失败
unsigned char tcp_server_init(void)
{
	unsigned char res;
	unsigned int cpu_sr;
	
//	OS_ENTER_CRITICAL();	//关中断
//	res = OSTaskCreate(tcp_server_thread,(void*)0,(unsigned int*)&TCPSERVER_TASK_STK[TCPSERVER_STK_SIZE-1],TCPSERVER_PRIO); //创建TCP服务器线程
//	OS_EXIT_CRITICAL();		//开中断
